return {
    ...
    dependencies = vim.fn.has("nvim-0.11.0") == 1 and "williamboman/mason-lspconfig.nvim"
        or { "williamboman/mason-lspconfig.nvim", "hrsh7th/cmp-nvim-lsp" },
    lazy = false,
    ...
    opts = {
        ...
        on_attach = function(client, bufnr)
            ...
            if
                (vim.fn.has("nvim-0.11.0") == 1 and client:supports_method(methods.textDocument_inlayHint, bufnr))
                or client.supports_method("textDocument/inlayHint", { bufnr = bufnr })
            then
                lsp.inlay_hint.enable(true, { bufnr = bufnr })
            end

            if
                (vim.fn.has("nvim-0.11.0") == 1 and client:supports_method(methods.textDocument_codeLens, bufnr))
                or client.supports_method("textDocument/codeLens", { bufnr = bufnr })
            then
            ...
            if vim.fn.has("nvim-0.11.0") == 1 and client:supports_method(methods.textDocument_completion, bufnr) then
                local completion = require("utils.completion")

                completion.enable(true, client.id, bufnr, {
                    autotrigger = true,
                    convert = completion.format_completion(client, bufnr),
                })
