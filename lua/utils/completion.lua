...
M.enable_documentation = function(client, bufnr)
    ...
    vim.api.nvim_create_autocmd("CompleteChanged", {
        ...
        callback = function()
            ...
            client:request("completionItem/resolve", completionItem, function(err, result)
                if err then
                    if vim.g.documentation_win and vim.api.nvim_win_is_valid(vim.g.documentation_win.winid or -1) then
                        vim.api.nvim_win_close(vim.g.documentation_win.winid, false)
                        vim.g.documentation_win = nil
                    end
                    return
                end
                ...
                local win_doc = result.documentation
                        and vim.api.nvim__complete_set(info["selected"], { info = result.documentation.value })
                    or nil
                ...
                vim.treesitter.start(win_doc.bufnr, result.documentation.kind)
