...
setmetatable(M, {
    __index = function(t, k)
        t[k] = require("utils.plugins.mason." .. k)
        return t[k]
    end,
})
...
M.check_and_notify_bad_setup_order = function(extension)
    ...
    local impacts_functionality = not mason_ok
        or #require("mason-" .. extension .. ".settings").current.ensure_installed
            > 0
    return function()
        if is_bad_order and impacts_functionality then
            vim.notify(
                ("mason.nvim has not been set up. Make sure to set up 'mason' before 'mason-%s'."):format(
                    extension
                ),
                vim.log.levels.WARN,
                {
